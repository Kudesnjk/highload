...
--- | ---
Видео контент | (6168 * 6 000 000) / (24 * 60 * 60)  = **0.41** тбит / c
Статика | (672 * 6 000 000) / (24 * 60 * 60) = **45.5** гбит / c
Бизнес-логика | (1104 * 6 000 000) / (24 * 60 * 60) = **75** гбит / c 
...
Итого, итоговый размер БД на год равен:
5 + 7,45 + 66,6 + 30 + 580 + 940 = **1629** гб = **1.6** тб 
...
Когда контент становиться "холодным", то его можно переместить в хранилище, где он будет храниться на RAID-10 массиве HDD.
Объёмы хранилища на год = 1163 * 12 = **13.6** пб

#### Подсчёт количества оборудования

Сервер | CPU | RAM | Тип диска | Объём диска| Количество дисков | Количество серверов
--- | --- | --- | --- | --- | --- | --- 
Nginx | 16 | 16 GB | - | - | - | 50
Backend | 16 | 32 GB | - | - | - | 2
PostgreSQL | 16 | 32 GB | SSD | 2 тб | 4 (RAID10) | 3 (1 master + 2 slaves)
CDN | 16 | 16 GB | SSD | 2 TB | 4 (RAID10) | 16
